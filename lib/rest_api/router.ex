defmodule RestApi.Router do
  ...
  plug(Plug.Logger)
  plug(:match)
  plug(:basic_auth, Application.compile_env(:rest_api, :basic_auth))
  ...
  get "/knockknock" do
    case Mongo.command(:mongo, ping: 1) do
      ...
      {:error, _res} -> send_resp(conn, 500, "Something went wrong")
    end
  end
  ...
  get "/posts" do
    posts =
      ...
      |> Enum.map(&JSON.normalizeMongoId/1)
      |> Enum.to_list()
      |> Jason.encode!()
    ...
    case conn.body_params do
      %{"name" => name, "content" => content} ->
        case Mongo.insert_one(:mongo, "Posts", %{"name" => name, "content" => content}) do
          {:ok, post} ->
            doc = Mongo.find_one(:mongo, "Posts", %{_id: post.inserted_id})

            post =
              JSON.normalizeMongoId(doc)
              |> Jason.encode!()
